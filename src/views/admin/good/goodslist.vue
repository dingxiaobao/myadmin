<template>
  <div>
    ...
    <el-card>
      <!-- 提示区域 -->
      <el-alert title="添加商品信息" type="info" center shpw-icon :closable="false"></el-alert>
      <!-- <div class="box">
        <p>添加商品信息</p>
      </div>-->
      <!-- 步骤条区域 -->
...
export default {
  ...
  data() {
    ...
    var checkSL = (rules, val, cb) => {
      var reg = /^\+?[1-9]\d*$/;
      if (!reg.test(val)) {
        cb(new Error("商品数量必须是大于0的整数"));
      } else {
        cb();
      }
    };
    var checkCL = (rules, val, cb) => {
      var reg = /^\+?[1-9]\d*$/;
      if (!reg.test(val)) {
        cb(new Error("商品重量必须是大于0的整数"));
      } else {
        cb();
      }
    };
    return {
      ...
      addFormRules: {
        ...
        goods_weight: [
          ...
          { validator: checkCL, trigger: "blur" }
        ],
        goods_number: [
      ...
      // 图片上传组件的herders请求头对象
      headerObj: {
        Authorization: window.sessionStorage.getItem("04A_token")
      },
      previewPath: "",
